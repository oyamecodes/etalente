...
`docker compose up --build` brings up both services. Topology: frontend (nginx:alpine serving the `flutter build web` output) on host `:8080` → reverse-proxies `/api`, `/actuator`, `/v3/api-docs`, `/swagger-ui` to backend (Temurin 21 JRE running the Spring Boot fat jar) on host `:8081`. Override ports with `FRONTEND_PORT` / `BACKEND_PORT`. The frontend image is built with `--build-arg API_BASE_URL=""` so the SPA uses relative URLs — that's why `ApiClient.defaultBaseUrl` must treat "defined empty" as "same-origin" (see `frontend/lib/core/api/api_client.dart`). To ship the frontend against a remote API instead, pass `--build-arg API_BASE_URL=https://...` at build time. Compose's `depends_on.condition: service_healthy` gates frontend on the backend's actuator healthcheck. Default env: `APP_AUTH_MODE=dev`, `ASSISTANT_PROVIDER=canned` — zero-config. Never commit a `.env` with real Firebase credentials.
...
- API base URL is resolved at build time via `--dart-define=API_BASE_URL=...`. Defaults: `http://10.0.2.2:8080` on Android (emulator → host loopback), `http://localhost:8080` elsewhere. Override for physical devices / deployed backends. An **explicitly empty** `API_BASE_URL` (defined-but-empty, not absent) forces same-origin relative URLs — that's the mode the Docker frontend image uses so nginx can reverse-proxy `/api` to the backend. `ApiClient.defaultBaseUrl` uses `bool.hasEnvironment` to tell "empty" apart from "unset".
